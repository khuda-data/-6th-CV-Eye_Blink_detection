...
st.image(r"C:\Users\degas\Desktop\최종 CV 프로젝트\logo.png", width=600)#, use_container_width=True)
...
if menu == "📸 자동 촬영":
    ...
    if start_capture:
        ...
        # 해상도 및 프레임 속도 설정
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)  # 해상도 너비
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)  # 해상도 높이
        ...
        while cap.isOpened():
            ...
            if all_eyes_open and len(faces) > 0:
                ...
                if elapsed_time <= 2:
                    countdown = int(2 - elapsed_time) + 1
                    cv2.putText(frame, f"Capturing in {countdown}", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 2, (0, 255, 255), 3)
                elif elapsed_time > 2:
                    filename = save_photo(frame)
                    st.image(filename, caption="📸 촬영된 사진", use_container_width="auto")
    ...
    if st.button("📸 Capture Photo"):
        ...
        if ret:
            filename = save_photo(frame)
            if filename:
                st.image(filename, caption="📷 Captured Photo", use_container_width="auto")
                st.success(f"✅ 사진이 저장되었습니다: {filename}")
            else:
    ...
    if image_files:
        ...
        for idx, img_file in enumerate(image_files):
            with cols[idx % 3]:
                st.image(img_file, caption=f"🖼️ {os.path.basename(img_file)}", use_container_width=True)
